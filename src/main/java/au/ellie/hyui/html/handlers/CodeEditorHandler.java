...
import au.ellie.hyui.builders.CodeEditorBuilder;
...
public class CodeEditorHandler implements TagHandler {
    @Override
    public boolean canHandle(Element element) {
        if (element.tagName().equalsIgnoreCase("code-editor")) {
            return true;
        }
        return element.tagName().equalsIgnoreCase("textarea")
                && (element.hasClass("code-editor") || element.hasAttr("data-hyui-code-editor"));
    }

    @Override
    public UIElementBuilder<?> handle(Element element, HtmlParser parser) {
        CodeEditorBuilder builder = CodeEditorBuilder.codeEditor();
        applyCodeEditorAttributes(builder, element);
    ...
    private void applyCodeEditorAttributes(CodeEditorBuilder builder, Element element) {
        ...
        if (element.hasAttr("data-hyui-max-lines")) {
            ParseUtils.parseInt(element.attr("data-hyui-max-lines"))
                    .ifPresent(builder::withMaxLines);
        }
        if (element.hasAttr("data-hyui-auto-grow")) {
            builder.withAutoGrow(Boolean.parseBoolean(element.attr("data-hyui-auto-grow")));
        }
        if (element.hasAttr("data-hyui-auto-focus")) {
            builder.withAutoFocus(Boolean.parseBoolean(element.attr("data-hyui-auto-focus")));
        }
        if (element.hasAttr("data-hyui-auto-select-all")) {
            builder.withAutoSelectAll(Boolean.parseBoolean(element.attr("data-hyui-auto-select-all")));
        }
        if (element.hasAttr("data-hyui-language")) {
            builder.withLanguage(element.attr("data-hyui-language"));
        }
        if (element.hasAttr("data-hyui-line-number-width")) {
            ParseUtils.parseInt(element.attr("data-hyui-line-number-width"))
                    .ifPresent(builder::withLineNumberWidth);
        }
        if (element.hasAttr("data-hyui-line-number-padding")) {
            ParseUtils.parseInt(element.attr("data-hyui-line-number-padding"))
                    .ifPresent(builder::withLineNumberPadding);
        }
        if (element.hasAttr("data-hyui-line-number-text-color")) {
            builder.withLineNumberTextColor(element.attr("data-hyui-line-number-text-color"));
        }
        if (element.hasAttr("data-hyui-line-number-background")) {
            parseStyleReference(element.attr("data-hyui-line-number-background"))
                    .ifPresent(ref -> builder.withLineNumberBackground(ref.document, ref.reference));
        }
